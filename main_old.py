...
def animate(message: str, steps: int = 3, delay: float = 0.4):
...
def main():
    ...
    # ========================================================================
    # STEP 5: Simulate corruption (errors) – with AWGN option
    # ========================================================================
    print("-" * 70)
    ...
    if n_errors > 0:
        ...
        if mode == "1":
            ...
            # AWGN-like noise: add Gaussian noise and clip to [0,255]
            for pos in corruption_positions:
                original_value = corrupted_bytes[pos]
                noise = random.gauss(0.0, noise_sigma)
                noisy_val = int(round(original_value + noise))
                noisy_val = max(0, min(255, noisy_val))
                corrupted_bytes[pos] = noisy_val
                print(f"  position {pos:3d}: {original_value:3d}  + N(0,{noise_sigma})  →  {noisy_val:3d}")
    ...
    print(f"Corruption model:          {'XOR random errors' if mode == '1' else f'AWGN-like (σ={noise_sigma})'}")
